from s_testing_lab.d_4_car_manager import Car
...
class TestCar(TestCase):
    ...
    def test_car_set_fuel_consumption_less_than_or_equal_to_zero(self):
        with self.assertRaises(Exception) as ex:
    ...
    def test_car_set_fuel_capacity_less_than_or_equal_to_zero(self):
        with self.assertRaises(Exception) as ex:
    ...
    def test_car_set_fuel_amount_less_than_or_equal_to_zero(self):
        with self.assertRaises(Exception) as ex:
    ...
    def test_refuel_greater_than_capacity_sets_to_capacity(self):
        self.assertEqual(0,self.car.fuel_amount)
        fuel_amaunt = self.car.fuel_capacity+1
        self.car.refuel(fuel_amaunt)
        self.assertEqual(self.car.fuel_capacity, self.car.fuel_amount)

    def test_drive_not_enough_fuel_amaount(self):
        self.assertEqual(0,self.car.fuel_amount)
